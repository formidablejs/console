...
const ΨapplicationEvents = Symbol.for('#applicationEvents'), ΨonDefaultCommandEvents = Symbol.for('#onDefaultCommandEvents'), Ψ__init__ = Symbol.for('#__init__'), Ψ__initor__ = Symbol.for('#__initor__'), Ψ__inited__ = Symbol.for('#__inited__');
...
class Application {
	[Ψ__init__]($$ = null){
		...
		this[ΨapplicationEvents] = ($$ && (vφ = $$[ΨapplicationEvents]) !== undefined) ? (vφ) : [];
	...
	*/
	onEvent(event,callback){
		...
		this[ΨapplicationEvents].push({event: event,callback: callback});
	...
	*/
	run(signature = null){
		...
		if (results == 0) {
			...
			for (let iφ2 = 0, itemsφ3 = iter$__(this[ΨonDefaultCommandEvents]), lenφ3 = itemsφ3.length; iφ2 < lenφ3; iφ2++) {
		...
		if (options.name) {
			...
			for (let iφ3 = 0, itemsφ4 = iter$__(this[ΨapplicationEvents].filter(function(event) { return event.event == ("pre-" + (options.name)); })), lenφ4 = itemsφ4.length; iφ3 < lenφ4; iφ3++) {
				let registered = itemsφ4[iφ3];
		...
		command.run(options,(results instanceof _$GlobalOptionsφ.default) ? results : undefined);
		
		if (options.name) {
			
			resφ = [];
			for (let iφ4 = 0, itemsφ5 = iter$__(this[ΨapplicationEvents].filter(function(event) { return event.event == options.name; })), lenφ5 = itemsφ5.length; iφ4 < lenφ5; iφ4++) {
				let registered = itemsφ5[iφ4];
				resφ.push(registered.callback(options,results));
			};
			return resφ;
		};
	}
