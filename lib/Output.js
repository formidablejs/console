...
/*body*/
var $1 = require('console'/*$path$*/);
var $2 = require('stream'/*$path$*/);
...
class Output {
	...
	*/
	static table(array,results = ''){
		...
		const transform = new $2.Transform({
			transform: function(chunk,enc,cb) { return cb(null,chunk); }
		});
		
		const logger = new $1.Console({stdout: transform});
		
		logger.table(array);
		
		const table = (transform.read() || '').toString();
		
		for (let $3 = 0, $4 = iter$__(table.split(/[\r\n]+/)), $5 = $4.length; $3 < $5; $3++) {
			let row = $4[$3];
			let r = row.replace(/[^┬]*┬/,'┌');
			r = r.replace(/^├─*┼/,'├');
			r = r.replace(/│[^│]*/,'');
	...
	*/
	static write(line){
		...
		return console.log(this.noAnsi ? line.replace(/\u001b\[.*?m/g,'') : line);
	}
...
exports.default = Output; Output[$__init__$]();
