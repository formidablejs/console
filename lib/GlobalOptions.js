...
class GlobalOptions {
	...
	*/
	constructor(help = false,quiet = false,noInteraction = false,env = null,noAnsi = false,verbose = 1){
		this[$__init__$]();
		this.help = help;
		this.quiet = quiet;
		...
		this.incoming = process.argv.slice(2).filter(function(arg) {
			...
			return !(['--help','-h','--quiet','-q','--no-interaction','-n','env','--no-ansi','-v','-vv','-vvv','-verbose'].includes(arg));
		});
	}
};
